...
# Comments added for study

# To-do
# - Name or distinguish between two human players
# - Computer response to corner trap is flawed --> response must be center
# ? Keep board clear of position numbers ("O" can be confused for 0)
# * Computer v. Computer results in a draw
...
module TicTacToe
  ...
  class Game
    # Arguments specify whether player is human or computer
    def initialize(player_1_class, player_2_class)
      # Board will be stored in an array
      @board = Array.new(10) # Ignore index 0 for convenience

      @current_player_id = 0 # Will alternate between 0 and 1
    ...
    def turn_num # Used by ComputerPlayer
      10 - free_positions.size # Returns 1 before markers are placed
    end

    def print_board
      col_separator, row_separator = " | ", "--+---+--"
      # Return marker in given position or position number if nil
      label_for_position = lambda{|position| @board[position] ? @board[position] : position}
      # Label a row of positions separated by col_separator
      row_for_display = lambda{|row| row.map(&label_for_position).join(col_separator)}
      row_positions = [[1,2,3],[4,5,6],[7,8,9]]
      rows_for_display = row_positions.map(&row_for_display)
      # Join each element (row) with row_separator
  ...
  class ComputerPlayer < Player
    ...
    # Returns counts of markers/empty spaces for each winning line
    def group_positions_by_markers(line) 
      markers = line.group_by {|position| @game.board[position]}
      markers.default = [] # Rather than nil
      markers
    end
    ...
    def look_for_winning_or_blocking_position(opponent_marker)
      for line in LINES
        ...
        next if markers[nil].length != 1 # Ignore line unless only 1 space is left
    ...
    def corner_trap_defense_position(opponent_marker)
      ...
      return safe_responses[opponent_position].sample # Random selection
    end
